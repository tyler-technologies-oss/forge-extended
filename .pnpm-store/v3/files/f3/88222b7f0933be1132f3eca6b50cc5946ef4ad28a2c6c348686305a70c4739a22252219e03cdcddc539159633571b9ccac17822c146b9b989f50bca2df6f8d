...
exports.locationCorrectionWalker = void 0;
const util_js_1 = require("./util.js");
const correctLocation = (node, loc, baseIndentations, prefixOffsets, placeholders) => {
    ...
    for (let i = 0; i < node.quasi.expressions.length; i++) {
        ...
        if (expr &&
            ...
            const placeholder = placeholders?.get(i) ?? (0, util_js_1.createPlaceholder)(i);
 ...
 * @return {Function}
 */
function locationCorrectionWalker(expr) {
    return (node) => {
        ...
        if (node.source?.start) {
            node.source.start = correctLocation(expr, node.source.start, baseIndentations, root.raws['litPrefixOffsets'], root.raws['litPlaceholders']);
        }
        if (node.source?.end) {
            node.source.end = correctLocation(expr, node.source.end, baseIndentations, root.raws['litPrefixOffsets'], root.raws['litPlaceholders']);
        }
    };
}
