...
- Must be explicitly opened via `openRightDrawer()` or clicking an element with `data-forge-app-layout-right`
...
Unlike the navigation drawer which has a built-in menu button, the right drawer requires a trigger element. Add the `data-forge-app-layout-right` attribute to any element to make it toggle the right drawer:
...
The `APP_LAYOUT_RIGHT_TOGGLE_ATTRIBUTE` constant is exported for use in JavaScript/TypeScript:

```typescript
import { APP_LAYOUT_RIGHT_TOGGLE_ATTRIBUTE } from '@tylertech/forge-extended';

element.setAttribute(APP_LAYOUT_RIGHT_TOGGLE_ATTRIBUTE, '');
```

### Programmatic Control

You can also control the right drawer programmatically using the following methods:
