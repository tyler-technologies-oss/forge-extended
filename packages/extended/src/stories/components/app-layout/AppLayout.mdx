...
You can also programmatically open or close the drawer using the `openDrawer()` and `closeDrawer()` methods:
...
In some cases, you may want to use the app-layout component without the built-in app bar, such as when embedding the layout within another application or using a custom header. You can hide the app bar by setting the `no-app-bar` attribute:

```html
<forge-app-layout no-app-bar>
...
> **Note:** When the app bar is hidden, the built-in menu button for toggling the navigation drawer on small screens will also be hidden. You will need to provide your own button and use the `openDrawer()` method to open the navigation drawer on small screens.

```html
<forge-app-layout id="app-layout" no-app-bar>
...
The `preset` property allows you to configure the app-layout for different application types. Each preset applies a CSS state that can be used for targeted styling.

### Available Presets

- **backoffice** (default): For internal business applications and administrative interfaces
- **public**: For public-facing applications and customer portals
- **documentation**: For documentation sites and knowledge bases (app bar is not rendered)

```html
<!-- Default backoffice preset -->
...
You can use CSS custom states to apply preset-specific styles:

```css
forge-app-layout:state(backoffice) {
