...
By default, the app-layout component is configured to fill the entire viewport using the `viewport` attribute on the underlying `<forge-scaffold>` component. This ensures the layout stretches to fill the full browser window height.

### Disabling Viewport Mode

In some cases, you may want the layout to behave as a normal block-level element rather than filling the viewport. You can disable viewport mode by setting `viewport="false"`:

```html
<forge-app-layout app-title="My Application" viewport="false">
  <!-- Navigation -->
...
When `viewport` is set to `false`:

- The layout will not stretch to fill the viewport height
- The layout will grow based on its content
- Useful when embedding the app-layout within another layout structure
- Provides more flexibility for non-full-page applications

> **Note:** In most Tyler Cloud applications, the default `viewport="true"` behavior is recommended for a consistent full-page application experience.
