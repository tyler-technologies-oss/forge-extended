...
that is running. When the process is canceled, the busy indicator will dispatch a `forge-ext-busy-indicator-cancel` event that can be used to handle the cancellation
