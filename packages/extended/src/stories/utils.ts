...
export function transformCssPropsToControls(tagName: string) {
  const declaration = cem.modules
    ...
    .find((declaration: any) => declaration.tagName === tagName);
...
export function applyArgs(
  ...
  props: Partial<HTMLElement> & { [key: string]: any }
) {
...
export function generateCustomElementArgTypes({
  ...
  if (cssProperties.length) {
    const cssPropertyArgTypes = generateArgTypesFrom(
      cssProperties,
      category ? `${category} css custom properties` : 'css custom properties',
      'text'
    );
...
export function getCustomElementsTagModule(tagName: string): any {
  return cem.modules.find((module: any) =>
    module.declarations.some((declaration: any) => declaration.tagName === tagName)
  );
}
